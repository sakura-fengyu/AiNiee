...
class RequestTester(Base):
    ...
    def api_test(self, event, data: dict):
        ...
        if proxy_enable == False or proxy_url == "":
            ...
            self.info(format('系统代理已启用，代理地址：{proxy_url}'))
        ...
        for api_key in api_keys:
            ...
            self.print("")
            self.info("正在进行接口测试 ...")
            self.info(format('接口名称 - {name}'))
            self.info(format('接口地址 - {api_url}'))
            self.info(format('接口密钥 - {api_key}'))
            self.info(format('模型名称 - {model}'))
            self.info(format('额外参数 - {extra_body}'))
            self.info("测试指令 - ")
            self.print(format('{messages}'))
            ...
            try:
                ...
                # 打印日志
                self.info("接口测试成功 ...")
                self.info(format('接口返回信息 - {content}'))

                # 储存结果
                success.append(api_key)
            except Exception as e:
                # 打印日志
                self.error(format('接口测试失败 ... {e}'))
        ...
        # 打印结果
        self.print("")
        self.info(format('接口测试结果：共测试 {len(api_keys)} 个接口，成功 {len(success)} 个，失败 {len(failure)} 个 ...'))
        if len(failure) >0:
            self.error(format('失败的接口密钥 - {", ".join(failure)}'))
        self.print("")
