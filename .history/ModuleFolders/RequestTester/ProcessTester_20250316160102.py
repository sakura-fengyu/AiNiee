...
class ProcessTester(Base):
    ...
    def _generic_process_phase(self, config, phase_flag):
        ...
        # 结构化输出请求参数
        self.info("[接口参数]")
        self.print(format('  → 接口地址: {api_url}'))
        self.print(format('  → 模型名称: {model_name}'))
        self.print(format('  → 额外参数: {extra_body}'))
        self.print(format('  → 接口密钥: {'*'*(len(api_key)-4)}{api_key[-4:]}'))  # 隐藏敏感信息
        ...
        # ================== 响应阶段日志 ==================
        self.print("")
        self.info(format('接口响应结果'))

        if response_think:
            self.info("[思考过程]\n")
            self.print(format('{response_think.strip()}\n'))

        self.info("[回复内容]\n")
        self.print(format('{response_content.strip()}\n'))
    ...
    def _call_api(self, api_format, api_key, api_url, model_name,extra_body, messages, system_content):
        ...
        try:
            ...
            self.error(format('{api_format} API调用失败'))
            raise
    ...
    def get_platform_config(self, platform):
        ...
        # 获取平台配置标识
        platform_tag = user_config.get(format('request_{platform}_platform_settings'))
        ...
        if proxy_enable == False or proxy_url == "":
            ...
            self.info(format('[系统代理]  {proxy_url}'))
    ...
    def log_rex_list(self):
        ...
        for i, (key, value) in enumerate(self.rex_list.items(), 1):
            ...
            # 首行特殊处理
            log_lines.append(format('{key_str} : {value_lines[0]}" if value_lines else f"{key_str} : '))
