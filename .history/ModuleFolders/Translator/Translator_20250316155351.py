...
class Translator(Base):
    ...
    def translation_manual_export(self, event: int, data: dict) -> None:
        ...
        if self.config.response_conversion_toggle:
            ...
            self.print("")
            self.info(format('已启动自动简繁转换功能，正在使用 {self.config.opencc_preset} 配置进行字形转换 ...'))
            self.print("")
    ...
    def translation_start_target(self, continue_status: bool) -> None:
        ...
        for current_round in range(self.config.round_limit + 1):
            ...
            # 输出开始翻译的日志
            self.print("")
            self.info(format('当前轮次 - {current_round + 1}'))
            self.info(format('最大轮次 - {self.config.round_limit}'))
            self.info(format('项目类型 - {self.config.translation_project}'))
            self.info(format('原文语言 - {self.config.source_language}'))
            self.info(format('译文语言 - {self.config.target_language}'))
            self.print("")
            if self.config.double_request_switch_settings == False:
                self.info(format('接口名称 - {self.config.platforms.get(self.config.target_platform, {}).get("name", "未知")}'))
                self.info(format('接口地址 - {self.config.base_url}'))
                self.info(format('模型名称 - {self.config.model}'))
                self.print("")
                self.info(format('生效中的 网络代理 - {self.config.proxy_url}')) if self.config.proxy_enable == True and self.config.proxy_url != "" else None
                self.info(format('生效中的 RPM 限额 - {self.config.rpm_limit}'))
                self.info(format('生效中的 TPM 限额 - {self.config.tpm_limit}'))
                ...
                self.print("")
                if system:
                    self.info(format('本次任务使用以下基础提示词：\n{system}\n')) 
                    
            else:
                self.info(format('第一次请求的接口 - {self.config.platforms.get(self.config.request_a_platform_settings, {}).get("name", "未知")}'))
                self.info(format('接口地址 - {self.config.base_url_a}'))
                self.info(format('模型名称 - {self.config.model_a}'))
                self.print("")

                self.info(format('第二次请求的接口 - {self.config.platforms.get(self.config.request_b_platform_settings, {}).get("name", "未知")}'))
                self.info(format('接口地址 - {self.config.base_url_b}'))
                self.info(format('模型名称 - {self.config.model_b}'))
                self.print("")
            ...
            self.info(format('即将开始执行翻译任务，预计任务总数为 {len(tasks_list)}, 同时执行的任务数量为 {self.config.actual_thread_counts}，请注意保持网络通畅 ...'))
            self.print("")
        ...
        self.print("")
        self.info(format('翻译结果已保存至 {self.config.label_output_path} 目录 ...'))
        self.print("")
