...
class PromptBuilder(Base):
    ...
    def build_translation_sample(config: TranslatorConfig, input_dict: dict) -> tuple[str, str]:
        ...
        if source_dict:
            ...
            for index_s, line in enumerate(source_dict.values()):
                source_numbered_lines.append(format('{index_s + 1}. {line}')) # 添加序号和 "." 分隔符

            target_numbered_lines = []
            for index_t, line in enumerate(target_dict.values()):
                target_numbered_lines.append(format('{index_t + 1}. {line}')) # 添加序号和 "." 分隔符
    ...
    def build_glossary_prompt(config: TranslatorConfig, input_dict: dict) -> str:
        ...
        if config.target_language in ("chinese_simplified", "chinese_traditional"):
            ...
            # 添加数据
            for v in result:
                glossary_prompt_lines.append(format('{v.get("src")}|{v.get("dst")}|{v.get("info") if v.get("info") != "" else " "}'))
