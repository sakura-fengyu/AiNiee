...
class ArgsEditPage(MessageBoxBase, Base):
    ...
    def add_widget_top_p(self, parent, config, preset):
        def init(widget):
            ...
            widget.set_text(format('{config.get("platforms").get(self.key).get("top_p"):.2f}'))
            widget.set_value(int(config.get("platforms").get(self.key).get("top_p") * 100))

        def value_changed(widget, value):
            widget.set_text(format('{(value / 100):.2f}'))
    ...
    def add_widget_temperature(self, parent, config, preset):
        def init(widget):
            ...
            widget.set_text(format('{config.get("platforms").get(self.key).get("temperature"):.2f}'))
    ...
    def add_widget_presence_penalty(self, parent, config, preset):
        def init(widget):
            ...
            widget.set_text(format('{config.get("platforms").get(self.key).get("presence_penalty"):.2f}'))
    ...
    def add_widget_frequency_penalty(self, parent, config, preset):
        def init(widget):
            widget.set_range(-200, 200)
            widget.set_text(format('{config.get("platforms").get(self.key).get("frequency_penalty"):.2f}'))
