...
class PlatformPage(QFrame, Base):
    ...
    def api_test_done(self, event: int, data: dict):
        ...
        if len(data.get("failure", [])) > 0:
            info_cont = self.tra("接口测试结果：成功") + format('{len(data.get("success", []))}')+ "......" + self.tra("失败") + format('{len(data.get("failure", []))}') + "......"
            self.error_toast("", info_cont)
        else:
            info_cont = self.tra("接口测试结果：成功") + f"{len(data.get("success", []))}"+ "......" + self.tra("失败") + format('{len(data.get("failure", []))}' + "......"
