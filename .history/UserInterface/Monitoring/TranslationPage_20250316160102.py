...
class TranslationPage(QWidget, Base):
    ...
    def update_time(self, event: int, data: dict) -> None:
        ...
        if total_time < 60:
            ...
            self.time.set_value(format('{total_time}'))
        elif total_time < 60 * 60:
            self.time.set_unit("M")
            self.time.set_value(format('{(total_time / 60):.2f}'))
        else:
            self.time.set_unit("H")
            self.time.set_value(format('{(total_time / 60 / 60):.2f}'))
        ...
        if remaining_time < 60:
            ...
            self.remaining_time.set_value(format('{remaining_time}'))
        elif remaining_time < 60 * 60:
            self.remaining_time.set_unit("M")
            self.remaining_time.set_value(format('{(remaining_time / 60):.2f}'))
        else:
            self.remaining_time.set_unit("H")
            self.remaining_time.set_value(format('{(remaining_time / 60 / 60):.2f}'))
    ...
    def update_line(self, event: int, data: dict) -> None:
        ...
        if line < 1000:
            ...
            self.line_card.set_value(format('{line}'))
        elif line < 1000 * 1000:
            self.line_card.set_unit("KLine")
            self.line_card.set_value(format('{(line / 1000):.2f}'))
        else:
            self.line_card.set_unit("MLine")
            self.line_card.set_value(format('{(line / 1000 / 1000):.2f}'))

        remaining_line = self.data.get("total_line", 0) - self.data.get("line", 0)
        if remaining_line < 1000:
            self.remaining_line.set_unit("Line")
            self.remaining_line.set_value(format('{remaining_line}'))
        elif remaining_line < 1000 * 1000:
            self.remaining_line.set_unit("KLine")
            self.remaining_line.set_value(format('{(remaining_line / 1000):.2f}'))
        else:
            self.remaining_line.set_unit("MLine")
            self.remaining_line.set_value(format('{(remaining_line / 1000 / 1000):.2f}'))
    ...
    def update_task(self, event: int, data: dict) -> None:
        ...
        if task < 1000:
            ...
            self.task.set_value(format('{task}'))
        else:
            self.task.set_unit("KTask")
            self.task.set_value(format('{(task / 1000):.2f}'))
    ...
    def update_token(self, event: int, data: dict) -> None:
        ...
        if token < 1000:
            ...
            self.token.set_value(format('{token}'))
        elif token < 1000 * 1000:
            self.token.set_unit("KToken")
            self.token.set_value(format('{(token / 1000):.2f}'))
        else:
            self.token.set_unit("MToken")
            self.token.set_value(format('{(token / 1000 / 1000):.2f}'))
        ...
        if speed < 1000:
            ...
            self.speed.set_value(format('{speed:.2f}'))
        else:
            self.speed.set_unit("KT/S")
            self.speed.set_value(format('{(speed / 1000):.2f}'))
    ...
    def update_stability(self, event: int, data: dict) -> None:
        ...
        self.stability.set_value(format('{stability_percent:.2f}'))
