...
class Jr_Tpp:
    ...
    def __Readxlsx(self, name):
        ...
        try:
            ...
            self.logger.error(format('读取xlsx文件失败: {name}'))
    ...
    def __Writexlsx(self, df, name, full=False, output_black=True):
        ...
        try:
            ...
            self.logger.error(format('写入xlsx文件失败: {name}'))
            self.logger.error(traceback.format_exc())
            print(e)
    ...
    def ReadGame(self, GameDir: str):
        ...
        """
        self.logger.info(format('开始读取游戏目录: {GameDir}'))
                ...
                if name not in self.BlackFiles:
                    ...
                    try:
                        ...
                        try:
                            ...
                            self.logger.error(format('读取文件 {name} 失败'))
                            self.logger.error(traceback.format_exc())
                            print(
    ...
    def InjectGame(
        ...
        """
        self.logger.info(format('开始注入翻译到游戏目录: {GameDir}, 输出路径: {path}'))
        ...
        if untrsline:
            ...
            print(format('存在未翻译行，已应用原文并导出为“{output_path}”以供确认'))
            ...
            if len(name_parts) > 1 and name_parts[-1].endswith(".json"):
                ...
                if os.sep + "data" + os.sep in File and name.endswith(".json"):
                    ...
                    try:
                        ...
                        self.logger.error(format('写入文件 {name} 失败'))
                        self.logger.error(traceback.format_exc())
                        print(format('写入{name}失败'))
    ...
    def GetFileNames(self) -> list:
        ...
        if mapname:  # 检查 mapname 列表是否为空
            ...
            namelist.append(format('Map{mapname[0]}~{mapname[-1]}.json'))
        return namelist
    ...
    def ToXlsx(self, name: str, path: str, output_black=True):
        ...
        try:
            ...
            print(format('正在导出{outputname} to {output_dir}'))  # 打印导出路径
        except Exception as e:
            self.logger.error(format('导出xlsx文件 {outputname} 失败'))
            self.logger.error(traceback.format_exc())
            print(e)
            input("导出失败，请关闭所有xlsx文件后再次尝试")
    ...
    def ToCsv(self, name: str, path: str):
        ...
        try:
            ...
            print(format('正在保存{outputname} to {output_dir}'))  # 打印保存路径
        except Exception as e:
            self.logger.error(format('保存csv文件 {outputname} 失败'))
            self.logger.error(traceback.format_exc())
            print(e)
        ...
        if path:
            ...
            try:
                ...
                try:
                    ...
                    self.logger.error(format('从JSON文件导入翻译失败: {path}'))
                    self.logger.error(traceback.format_exc())
                    print(
        ...
        if trsdata:  # 确保 trsdata 不是 None
            ...
            for name in namelist:
                if name not in self.ProgramData:  # 检查 name 是否存在于 ProgramData 中
                    self.logger.warning(format('跳过文件 {name}，因为它不在工程数据中'))
                    continue
                ...
                if data is not None:  # 检查数据是否成功读取
                    if not samefile:
                        ...
                        if jsonname in self.ProgramData.keys():
                            ...
                            print(format('{name}没有对应的json文件，已跳过导入该文件'))
                else:
                    print(format('导入{name}失败，请检查文件'))
    ...
    def load(self, path: str):
        ...
        if not os.path.exists(project_data_path):  # 检查路径是否存在
            self.logger.warning(format('工程数据路径 {project_data_path} 不存在，加载失败'))
            print(format('工程数据路径 {project_data_path} 不存在，加载失败'))
            return
            ...
            if name.endswith(".csv"):
                ...
                if os.path.exists(csv_file_path):  # 再次检查文件是否存在
                    ...
                    try:
                        ...
                        self.logger.error(format('加载CSV文件 {file} 失败'))
                        self.logger.error(traceback.format_exc())
                        print(format('加载 {file} 失败'))
                else:
                    self.logger.warning(format('CSV 文件 {csv_file_path} 不存在'))
                    print(format('CSV 文件 {csv_file_path} 不存在'))
        ...
        for name, indices in target.items():  # 遍历字典的键值对
            ...
            for index in indices:  # 遍历索引列表
                if index in self.ProgramData[name].index:  # 检查索引是否存在
                    ...
                    print(format('在{name}中没有找到原文为:"{index}"的行'))
        print(format('添加标签"{label}"完成'))
            ...
            for index in indices:  # 遍历索引列表
                if index in self.ProgramData[name].index:  # 检查索引是否存在
                    ...
                    if label in self.ProgramData[name].loc[index, "标签"]:
                        ...
                        print(format('{name}:"{index}"行没有{label}标签'))
                else:
                    print(format('在{name}中没有找到原文为:"{index}"的行'))
        print(format('去除标签"{label}"完成'))
    ...
    def search(
        ...
        if col == 0:
            ...
            self.logger.error(format('无效的搜索列索引: {col}'))
            print(format('无效的搜索列索引: {col}'))
            return {}
        ...
        for name in namelist:  # 遍历文件名
            if name not in target:  # 检查 name 是否存在
                self.logger.warning(format('跳过文件 {name}，因为它不在搜索目标中'))
                print(format('{name}不在搜索目标范围内'))
                continue
    ...
    def Replace(
        ...
        for name in namelist:  # 遍历文件名列表
            if name not in target:  # 检查 name 是否存在
                self.logger.warning(format('跳过文件 {name}，因为它不在替换目标中'))
                print(format('{name}不在替换目标范围内'))
                continue
    ...
    def OutputBySearch(
        ...
        if res:
            ...
            print(format('已将搜索结果保存为{OutputName}'))
        else:
            print("搜索结果为空")
        return res
    ...
    def JsonBySearch(
        ...
        if res:
            ...
            try:
                ...
                print(format('已将结果导出为{OutputName}'))
            except Exception as e:
                self.logger.error(format('导出JSON文件 {OutputName} 失败'))
                self.logger.error(traceback.format_exc())
                print(format('导出JSON文件 {OutputName} 失败'))
            return output
        else:
            print("搜索结果为空")
            return {}
    ...
    def GetName(self, data_path, without: list = None):
        ...
        if self.ja and namedict:  # 检查 namedict 是否为空
            ...
            try:
                ...
                self.logger.error(format('导出JSON文件 Name.json 失败'))
                self.logger.error(traceback.format_exc())
                print(format('导出Name.json失败'))
    ...
    def dnb(self, GameDir, files: list = None):
        ...
        if self.need2check:
            ...
            try:
                ...
                print(format('已将可能需要人工修正的文本行导出到 {need2check_file}'))
            except Exception as e:
                self.logger.error(format('导出 need2check.json 文件失败'))
                self.logger.error(traceback.format_exc())
                print(format('导出 {need2check_file} 失败'))
    ...
    def checknum(self):
        ...
        try:
            ...
            self.logger.error(format('加载 {checkdict_file} 失败'))
            self.logger.error(traceback.format_exc())
            print(
        ...
        try:
            ...
            self.logger.error(format('导出 {checkres_file} 失败'))
            self.logger.error(traceback.format_exc())
            print(format('导出 {checkres_file} 失败'))
        print(format('已将检查结果保存到{checkres_file}'))
        ...
        if os.path.exists(fixdict_file):
            ...
            while inp not in ["y", "n"]:
                inp = input(format('{fixdict_file}已存在，请问要覆盖吗（y,n)\n')).lower()
            if inp == "y":
                overw = True
        if overw:
            try:
                ...
                self.logger.error(format('导出 {fixdict_file} 失败'))
                self.logger.error(traceback.format_exc())
                print(format('导出 {fixdict_file} 失败'))
    ...
    def fixnum(self):
        ...
        try:
            ...
            self.logger.error(format('加载 {checkres_file} 失败'))
            self.logger.error(traceback.format_exc())
            print(
        ...
        try:
            ...
            self.logger.error(format('加载 {fixdict_file} 失败'))
            self.logger.error(traceback.format_exc())
            print(
